...
function parseVariableDeclaration(declaration: ESTree.VariableDeclaration, currentNode: FlowNode, context: ParsingContext): Completion {
  for (let declarator of declaration.declarations) {
    let declarationExpression = declarator.init ? createAssignmentExpressionFrom(declarator) : declarator.id;

    currentNode = context.createNode().appendTo(currentNode, stringify(declarationExpression), declarator);
  }
